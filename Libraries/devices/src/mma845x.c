...
static struct mma845x_device mma845x_dev;


static const uint8_t mma845x_addr[] = {0x1C, 0x1D};

static int write_reg(uint8_t addr, uint8_t val)
{
    return I2C_WriteSingleRegister(mma845x_dev.instance, mma845x_dev.addr, addr, val);
}
...
int mma845x_init(uint32_t instance)
{
    ...
    mma845x_dev.instance = instance;
    
    for(i = 0; i < ARRAY_SIZE(mma845x_addr); i++)
    {
        if(!I2C_ReadSingleRegister(instance, mma845x_addr[i], MMA845x_WHO_AM_I, &id))
        {
            if(id == 0x1A)
            {
                //printf("MMA845x found!\r\n");
                mma845x_dev.addr = mma845x_addr[i];
                write_reg(CTRL_REG1,ASLP_RATE_20MS+DATA_RATE_1250US);
                write_reg(XYZ_DATA_CFG_REG, FULL_SCALE_2G); //2G
                write_reg(HP_FILTER_CUTOFF_REG, PULSE_LPF_EN_MASK ); //µÍÍ¨ÂË²¨
                write_reg(CTRL_REG1, ACTIVE_MASK);          //¼¤»î×´Ì¬
                return 0;
            }
        }
    }
...
int mma845x_read_data(int16_t *adata)
{
    ...
    err = I2C_BurstRead(mma845x_dev.instance, mma845x_dev.addr, MMA845x_OUT_X_MSB, 1, buf, 6);
